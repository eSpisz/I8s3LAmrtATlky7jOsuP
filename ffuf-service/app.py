...
def main():
    ...
    while True:
        ...
        if not ffuf.validate_target(target):
...
class Ffuf:
    ...
    def start_ffuf(self):
        ...
        self.ffuf_result = ffuf_scan.stdout
        logging.info(f"ffuf scan result: {self.ffuf_result}")
    ...
    def upload_result(self):
        ...
        try:
            s3.put_object(
                Bucket=BUCKET_NAME,
                Key=f'ffuf/{self.TARGET}.txt',
                Body=self.ffuf_result
            )
            
            logging.info(f"Uploaded ffuf result to S3: {self.TARGET}.txt")

        except ClientError as e:
            logging.error("Failed to upload ffuf result to S3: {e}")
